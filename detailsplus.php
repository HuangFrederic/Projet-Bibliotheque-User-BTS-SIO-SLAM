...
if(isset($_GET['id'])) {
    ...
    if($detailsLivre) {
        echo "Titre du livre : " . $detailsLivre['titre'] . "<br>";
        echo "Auteur : " . $detailsLivre['prenom'] . " " . $detailsLivre['nom'] . "<br>";

        // Appel de la fonction pour obtenir le nombre d'exemplaires disponibles
        $nombreExemplairesDisponibles = getCountExemplaire($id_livre);

        // Affichage du nombre d'exemplaires disponibles
        echo "Nombre d'exemplaires disponibles : " . $nombreExemplairesDisponibles;
    } else {
        echo "Aucun détail trouvé pour ce livre.";
    }
?>
    ...
    if (aEmprunte($_SESSION['id_user'], $id_livre)) {
        echo '<h3>Ajouter un commentaire</h3>
              <form method="post" action="#">
                  ...
                  Note (de 1 à 5) : <input type="number" name="note" min="1" max="5" required><br>
                  <input type="submit" name="comm" value="Envoyer">
              </form>';
    } else {
        echo "Vous devez emprunter ce livre pour pouvoir ajouter un commentaire.";
...
<tr>
  <td><?php echo $Comm['nom']; ?></td>
  <td><?php echo $Comm['prenom']; ?></td>
  <td><?php echo $Comm['note']; ?></td>
  <td><?php echo $Comm['commentaire']; ?></td>
  <td><?php echo $Comm['date_com']; ?></td>
</tr>
